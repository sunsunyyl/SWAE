...
def preprocess_fake_images(fid_ver, fake_images, norm=False):
    if np.shape(fake_images)[-1] == 1 and fid_ver == 'tf':
        fake_images = np.concatenate([fake_images, fake_images, fake_images], -1) 

    if np.shape(fake_images)[1] == 1 and fid_ver == 'pt':
        fake_images = np.concatenate([fake_images, fake_images, fake_images], -1) 
...
#tensorflow version
def evaluate_fid_score(args, model, results_dir, test_data, norm=False):

    fid_ver = 'tf'

    num_fid_data = args.num_fid_data

    if args.fid_score == True:
        fake_images = model.generate_x(num_fid_data)
        savefile = 'fid_result'
    elif args.fid_score_rec == True:
        fake_images = model.reconstruct_x(test_data[0:num_fid_data])
        savefile = 'fid_result_rec'


    fake_images = fake_images.detach().cpu().numpy()
    fake_images = fake_images.reshape(-1, args.input_size[1], args.input_size[2], args.input_size[0]).astype('float')

    real_images = test_data.detach().cpu().numpy()
    
    #np.random.shuffle(real_images)
    if args.dataset_name != 'coil20':
        real_images = real_images[0:5000]
    
    real_images = real_images.reshape(-1, args.input_size[1], args.input_size[2], args.input_size[0]).astype('float')


    real_images = preprocess_fake_images(fid_ver, real_images, norm)
    fake_images = preprocess_fake_images(fid_ver, fake_images, norm)
    ...
    print('real imags', np.shape(real_images))
    print('fake images', np.shape(fake_images))
    print('fid_result: ', fid_result)

    with open(results_dir + savefile + '.txt', 'w+') as f:
        print("real images = {s}".format(s = np.shape(real_images)), file = f)
        print("fake images = {s}".format(s = np.shape(fake_images)), file = f)
        print("fid_result = {fid_result}".format(fid_result = fid_result), file = f)
        print("time_elapsed = {time_elapsed}".format(time_elapsed = time_elapsed), file = f)
    f.close()

    print('time_elapsed: ', time_elapsed)

    np.savez(results_dir + savefile + '.npz', fid_result = fid_result)
