...
class VAE(Model):
    def __init__(self, args):
        ...
        if args.dataset_name.startswith('gmm'):
            self.q_z_logvar = NonLinear(300, self.args.z1_size, activation=None)
        else:
        ...
        if self.args.input_type == 'binary':
            ...
            if args.dataset_name.startswith('gmm'):
                self.p_x_mean = NonLinear(300, np.prod(self.args.input_size), activation=None)
                self.p_x_logvar = NonLinear(300, np.prod(self.args.input_size), activation=None)
            else:
                self.p_x_mean = NonLinear(300, np.prod(self.args.input_size), activation=nn.Sigmoid())
